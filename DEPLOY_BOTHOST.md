...
**Нода (Москва / Амстердам и др.):** если бот шлёт кракозябры вместо русского текста:
- **Node.js:** в `app.js` уже задаётся локаль UTF-8 при старте.
- **Python:** в `main.py` в начале задаётся `LANG`/`LC_ALL=en_US.UTF-8`; плюс в настройках бота на BOTHOST добавь переменные окружения **LANG** = `en_US.UTF-8` и **LC_ALL** = `en_US.UTF-8`.
- **Обязательно:** все файлы с русским текстом (в т.ч. `main.py`, `node_bot.js`) должны быть сохранены в кодировке **UTF-8**. В Cursor/VS Code: открой файл → меню «Файл» → «Сохранить с кодировкой» → выбери **UTF-8** → сохрани. Затем закоммить и задеплоить заново.
...
Если в «Логи работы» ничего нет или бот не отвечает:

1. **Библиотека и команда запуска должны совпадать:**
   - Выбрал **Node.js** → в настройках бота команда запуска: **`node app.js`** (файл в корне репозитория).
   - Выбрал **Python** → команда установки: **`pip install -r requirements.txt`**, команда запуска: **`python -u main.py`**.
2. **Git URL** в настройках бота должен быть ровно: `https://github.com/kazumaiq/cxner-music-bot.git` (с `.git` в конце, без пробелов).
3. **BOT_TOKEN** задан в переменных окружения (или в поле «Bot Token») — без токена бот не стартует.
4. Открой вкладку **«Логи сборки»** — если там ошибка (npm/pip, не найден файл), исправь команду или репозиторий.
5. После смены репозитория или команд нажми **Обновить/Перезапустить** и подожди 1–2 минуты, затем снова открой логи.

При успешном старте в логах появится строка `[entry] starting app.js -> node_bot.js` (Node) или сообщение о запуске бота (Python).

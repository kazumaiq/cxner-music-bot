...
**Нода (Москва / Амстердам и др.):** если на другой ноде бот шлёт кракозябры вместо русского текста, в коде уже задаётся локаль UTF-8 при старте (`app.js`). Убедись, что файлы в репозитории сохранены в кодировке **UTF-8** (в редакторе: «Сохранить с кодировкой» → UTF-8).
